...
const breadcrumbItems: BreadcrumbItem[] = [
    ...
    { title: 'Edit', href: `/master/personal-trainer-package/${props.pt_package.id}/edit` },
];

const form = useForm({
    gym_id: props.pt_package.gym_id,
    name: props.pt_package.name,
    duration_in_sessions: props.pt_package.duration_in_sessions,
    price: props.pt_package.price,
    max_person: props.pt_package.max_person,
    // Inisialisasi array promos dari relasi (asumsi nama relasi: pt_package_promos)
    promos: props.pt_package.pt_package_promos && props.pt_package.pt_package_promos.length > 0
        ? props.pt_package.pt_package_promos.map((p: any) => ({
...
const submit = () => {
    form.put(`/master/personal-trainer-package/${props.pt_package.id}`);
};
</script>
...
<template>
    <AppLayout :breadcrumbs="breadcrumbItems">
        ...
        <Head title="Edit Personal Trainer Package" />

        <div class="min-h-screen bg-muted/40 py-10">
            <div class="max-w-3xl mx-auto px-6">
                <div class="rounded-2xl border bg-background shadow-sm p-8 space-y-8">

                    <Heading variant="small" title="Edit Personal Trainer Package"
                        description="Perbarui informasi paket personal trainer dan promo." />
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="space-y-2">
                            <label class="text-sm font-medium">Durasi (Sesi)</label>
                            <Input v-model="form.duration_in_sessions" type="number" />
                        </div>
                        
                        <div class="space-y-2">
                            <label class="text-sm font-medium">Maksimal Peserta</label>
                            <Input v-model="form.max_person" type="number" />
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div v-for="(promo, index) in form.promos" :key="index"
                                ...
                                <button v-if="form.promos.length > 1" type="button" @click="removePromo(Number(index))"
                                ...
                                <div class="grid md:grid-cols-2 gap-4">
                                    <div class="space-y-2">
                                        ...
                                        <select v-model="promo.type"
                                            ...
                                            <option value="bonus_sessions">Bonus Sesi</option>
                                        </select>
                                    </div>
                                    <div class="space-y-2">
                        ...
                        <div class="flex justify-end items-center gap-4 pt-6 border-t">
                            <Link href="/master/personal-trainer-package" class="text-sm text-muted-foreground hover:underline">
                                Batal
                            </Link>
