...
const breadcrumbItems: BreadcrumbItem[] = [
    ...
    { title: 'Create', href: '/master/gym-class/create' },
];
...
<template>
    <AppLayout :breadcrumbs="breadcrumbItems">
        ...
        <Head title="Tambah Kelas Gym" />

        <div class="min-h-screen bg-muted/40 py-10">
            <div class="max-w-3xl mx-auto px-6">
                <div class="rounded-2xl border bg-background shadow-sm p-8 space-y-8">

                    <Heading variant="small" title="Tambah Kelas Gym Baru"
                        description="Buat kelas baru yang dapat dijadwalkan untuk booking member." />
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <Input v-model="form.name" placeholder="Contoh: Yoga Morning, HIIT Cardio" />
                            <p v-if="form.errors.name" class="text-xs text-destructive">{{ form.errors.name }}</p>
                        </div>

                        <div class="space-y-2">
                            <label class="text-sm font-medium">Deskripsi</label>
                            <textarea v-model="form.description" rows="3" placeholder="Deskripsi kelas..."
